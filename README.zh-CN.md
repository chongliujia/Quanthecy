...
3. 查看「新闻与证据」。独立新闻工作进程每 15 分钟检查两个选定的美联储官方资讯源。示例研究主题和事件档案需要管理员初始化，首次安装并不会自动包含截图中的全部内容。
...
除研究详情外，事件档案还组织范围定义、关联合约、证据和变化记录。文档保留发布时间、平台观测时间与历史版本。管理员针对“某个事件定义版本 + 某个文档版本”审核关联程度，版本更新后需要重新审核，详见[事件证据指南](docs/event-evidence.md)。
...
搜索美联储官方公告与讲话，按来源筛选，并设置研究截止时间，查看当时平台已保存的文档版本。每条记录分别展示发布时间与首次观测时间，标注版本和正文采集状态，并链接到官方来源。打开文档可检查已采集的正文与历史版本，详见[官方证据指南](docs/official-evidence.md)。
...
| 工作区、成员关系、中英文后台 | 关注列表与提醒、计费订阅、邀请和外部登录 |
