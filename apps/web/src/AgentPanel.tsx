...
export default function AgentPanel({ userId, organization, marketId, cutoff }: { userId: string; organization?: Organization; marketId: string; cutoff: string }) {
  ...
  return <section className="agent-panel">
    ...
    {status.data && <>
      ...
      {status.data.configuration_issue && <p className="data-warning">{t(status.data.configuration_issue === 'missing_key' ? 'Add an API key in Model settings before starting analysis.' : status.data.configuration_issue === 'invalid_context_window' ? 'Set the local model context window above the maximum output tokens.' : status.data.configuration_issue === 'missing_model' ? 'Add a model ID in Model settings before starting analysis.' : 'Check the provider and API address in Model settings.')}</p>}
