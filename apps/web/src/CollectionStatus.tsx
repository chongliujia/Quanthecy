import { t } from './i18n'
import { useCollectionStatus, age } from './collectionHealth'
...
export default function CollectionStatus({ userId }: { userId: string }) {
  ...
  return <div className="collection-strip" aria-label={t("Market data sources")}>
    ...
    {query.data?.sources?.map((source) => <details className={`source-health ${source.state !== 'recent' || source.error_code ? 'degraded' : ''}`} key={source.platform}>
      <summary><i /><strong>{source.platform}</strong><span>{source.error_code ? t("Request failed") : source.state === 'recent' ? t("Collecting") : source.state === 'delayed' ? t("Delayed") : t("No data")}</span><time title={time(source.latest_observation)}>{age(source.delay_seconds)}</time></summary>
      <div className="source-health-detail"><strong>{source.error_code ? t(reasons[source.error_code] ?? 'Collection interrupted') : source.state === 'recent' ? t("Recent observations available") : t("No recent observations received")}</strong><p>{t("Last observation:")} {time(source.latest_observation)}</p><p>{source.fresh_markets} {t("fresh open markets /")} {source.total_markets} {t("collected")}</p><p>{source.collector_checked_at ? t('Last collection attempt: {time}', { time: time(source.collector_checked_at) }) : t("Collector diagnostics unavailable. Freshness is based on saved observations.")}</p><small>{t("REST snapshots · Status checked")} {time(query.data.checked_at)}</small></div>
    </details>)}
  </div>
}
