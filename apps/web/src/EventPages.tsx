...
type Review = { id: string; relation: string; rationale: string; paragraphs: number[]; reviewed_at: string }
type Passage = { paragraph: number; text: string; truncated: boolean }
type Candidate = { id: string; evidence: Evidence; status: string; review: Review | null; passages: Passage[]; history: Review[]; matched_at: string }
export type EventDetail = { event: ResearchEvent; cutoff: string; markets: { id: string; platform: string; title: string; outcome: string; resolution_rules: string; closes_at: string | null; linked_at: string }[]; evidence: Candidate[]; counts: Record<string, number>; truncated: boolean; changes: { id: string; kind: string; title: string; observed_at: string; item_id: string | null; evidence_observed_at: string | null }[] }
  ...
  return <div className="event-research">
    ...
    {detail && <>
      ...
      <section className="panel" hidden={section !== 'evidence'}><div className="section-heading"><div><span className="eyebrow">{t('EVIDENCE REVIEW')}</span><h3>{t('Evidence for this event')}</h3></div><label>{t('Relevance filter')}<select value={filter} onChange={e => setFilter(e.target.value)}><option value="ALL">{t('All evidence')}</option>{Object.keys(detail.counts).map(s => <option key={s} value={s}>{statusLabel(s)} · {detail.counts[s]}</option>)}</select></label></div>
        ...
        {!detail.counts.DIRECT && <p className="data-warning">{t('No directly relevant evidence has been reviewed. Agent probability estimates remain unavailable; qualitative research can continue.')}</p>}
        <div className="event-evidence-list">{detail.evidence.filter(c => filter === 'ALL' || c.status === filter).map(candidate => <article className="event-evidence" key={candidate.id}><div className="evidence-meta"><span>{candidate.evidence.source_name} · v{candidate.evidence.version}</span><span className={`badge ${candidate.status === 'DIRECT' ? '' : 'badge-amber'}`}>{statusLabel(candidate.status)}</span></div><h4><a href={paragraphLink(candidate.evidence.id, candidate.evidence.observed_at)}>{candidate.evidence.title}</a></h4><small>{t('Published')} {time(candidate.evidence.published_at)} · {t('Observed')} {time(candidate.evidence.observed_at)}</small>
          {candidate.review ? <><p>{candidate.review.rationale}</p>{candidate.passages.map(p => <blockquote key={p.paragraph}><a href={paragraphLink(candidate.evidence.id, candidate.evidence.observed_at, p.paragraph)}>{t('Paragraph {number}', { number: p.paragraph })} ↗</a><p>{p.text}{p.truncated && '…'}</p></blockquote>)}<small>{t('Review recorded')} {time(candidate.review.reviewed_at)}</small></> : <p className="quiet">{t('Matched by official source. Relevance has not been approved by an operator.')}</p>}
          {candidate.history.length > 1 && <details><summary>{t('Review history')}</summary>{candidate.history.map(r => <p key={r.id}>{time(r.reviewed_at)} · {statusLabel(r.relation)} — {r.rationale}</p>)}</details>}
        </article>)}</div>
        {!detail.evidence.some(c => filter === 'ALL' || c.status === filter) && <p>{t('No evidence in this category.')}</p>}
        {detail.truncated && <p className="data-warning">{t('Showing the latest 100 evidence candidates. This view is not exhaustive.')}</p>}
      </section>
...
export function FrozenEvidence({ reference }: { reference: ResearchReference }) {
  ...
  const value = reference.value as { evidence?: { id?: string; observed_at?: string }; document_selection?: { passages?: Passage[] }; event_reviews?: { event_slug: string; review: Review }[] }
  const evidence = value.evidence
  if (!evidence?.id || !evidence.observed_at) return null
  return <div className="frozen-evidence-passages"><a href={paragraphLink(evidence.id, evidence.observed_at)}>{t('Open the cited evidence version →')}</a>{value.event_reviews?.map(v => <p key={v.review.id}>{statusLabel(v.review.relation)} · {v.review.rationale}</p>)}{value.document_selection?.passages?.map(p => <blockquote key={p.paragraph}><a href={paragraphLink(evidence.id!, evidence.observed_at!, p.paragraph)}>{t('Paragraph {number}', { number: p.paragraph })} ↗</a><p>{p.text}{p.truncated && '…'}</p></blockquote>)}</div>
}
