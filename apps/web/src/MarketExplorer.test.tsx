...
it('explains bounded coverage and empty history while sampling warms up', async () => {
  vi.spyOn(globalThis, 'fetch').mockImplementation(async () => new Response(JSON.stringify({ items: [], total: 0 })))
  mount()
...
it('scopes platform filtering and movement ranking to backend query parameters', async () => {
  const fetch = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => new Response(JSON.stringify({ items: [], total: 0 })))
  mount()
