...
function ConfigurationForm({ value, organizationId, userId }: { value: ModelConfiguration; organizationId: string; userId: string }) {
  ...
  const needsKey = draft.enabled && (draft.provider === 'anthropic' || draft.provider === 'openai' || (endpointProvider && endpointProvider.id !== 'local')) && !secret && (!value.has_api_key || clearKey || changingEndpoint)
  ...
  return <div className="settings-layout">
    <form id="model-configuration-form" className="panel model-settings" onSubmit={submit}>
      ...
      <div className="settings-actions"><span>{t('Saving does not call the model.')}</span><button className="primary" disabled={save.isPending || !allowed || wrongModelTarget || needsReplacement || !!needsKey || invalidLocalLimits}>{save.isPending ? t('Saving…') : t('Save configuration')}</button></div>
    </form>
