...
export default function PaperLab({ userId, organization }: { userId: string; organization?: Organization }) {
  ...
  return <WorkspacePaper key={organization.id} userId={userId} organization={organization} />
}

function WorkspacePaper({ userId, organization }: { userId: string; organization: Organization }) {
  ...
  const accountLabel = (id: string) => t(strategyNames[data?.accounts.find(a => a.id === id)?.strategy ?? ''] ?? '')
  ...
  return <div className="paper-lab">
    <div className="paper-intro"><div><span className="eyebrow">{t('VIRTUAL CAPITAL · REAL MARKET DATA')}</span><h2>{t('Paper trading lab')}</h2><p>{t('Compare fixed strategies with a complete record of simulated decisions, fills and costs.')}</p></div><span className="badge">{t('Simulation only')}</span></div>
