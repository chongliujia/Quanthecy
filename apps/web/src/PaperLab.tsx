...
function WorkspacePaper({ userId, organization, preselected }: { userId: string; organization: Organization; preselected: string | null }) {
  ...
  const comparisonForm = data !== undefined && data?.is_latest !== false && !lab.error && <AssistantComparison key={preselected ?? 'comparison'} userId={userId} organization={organization} current={data} preselected={preselected} onCreated={result => { setExperimentId(null); client.setQueryData(['paper', userId, organization.id, 'latest'], result); void client.invalidateQueries({ queryKey: ['paper', userId, organization.id] }) }} />
  ...
  return <div className="paper-lab">
    {!data && <div className="paper-intro"><div><span className="eyebrow">{t('VIRTUAL CAPITAL · REAL MARKET DATA')}</span><p>{t('Build research assistants and compare their simulated decisions, fills and costs.')}</p></div><span className="badge">{t('Simulation only')}</span></div>}
    {!data && <p className="paper-notice">{t('No real orders. Fills use later order-book snapshots, at most 10% of visible depth, observed fees and 10 bps of adverse slippage. Results are estimates.')}</p>}
    ...
    {data && <>
      ...
      <section className="paper-toolbar"><div><strong>{data.name}</strong><small>{data.market_count} {t('markets')} · {t('Started')} {time(data.created_at)}</small></div>{data.experiments && <label className="paper-history">{t('Experiment history')}<select aria-label={t('Experiment history')} value={experimentId ?? ''} onChange={e => { setExperimentId(e.target.value || null); setOrderId(null) }}><option value="">{t('Latest experiment')}</option>{data.experiments.map(e => <option key={e.id} value={e.id}>{e.version} · {time(e.created_at)}</option>)}</select></label>}<span className="badge simulation-label">{t('Simulation only')}</span><span className={`badge ${delayed ? 'paper-warning' : ''}`}>{delayed ? t('Processing delayed') : data.running ? t('Running') : t('Paused')}</span><button disabled={!writable || toggle.isPending || data.is_latest === false} onClick={() => toggle.mutate(!data.running)}>{data.running ? t('Pause experiment') : t('Resume experiment')}</button></section>
      ...
      {data.review_summary && !data.review_summary.model_ready && <p className="paper-model-status"><span className="status-dot" />{t('Model not configured · Agent reviews unavailable')}</p>}
      ...
      <p className="quiet paper-capital-note">{t('Platform balances are separate. Every strategy starts with the same virtual capital and market universe.')}</p>
      <div className="paper-accounts">{accounts.map(a => <article className="panel paper-account" key={a.id}><h3>{a.label || t(strategyNames[a.strategy])}</h3><span className="quiet">{t('Estimated liquidation equity')}</span><strong className="paper-equity">{cash(a.equity)}</strong><span className={valueTone(a.equity == null ? null : Number(a.equity) - Number(a.initial_cash))}>{a.equity == null ? t('Valuation unavailable') : `${cash(Number(a.equity) - Number(a.initial_cash))} (${((Number(a.equity) / Number(a.initial_cash) - 1) * 100).toFixed(2)}%)`}</span>
        ...
        {a.review_summary && <details className="account-review-details"><summary>{t('Review statistics')}</summary><dl><dt>{t('Participation in matched baseline entries')}</dt><dd>{a.review_summary.participation == null ? '—' : `${(a.review_summary.participation * 100).toFixed(1)}%`}</dd><dt>{t('Average signal-to-report time')}</dt><dd>{a.review_summary.average_latency_seconds == null ? '—' : `${a.review_summary.average_latency_seconds.toFixed(1)} s`}</dd><dt>{t('Provider calls attempted')}</dt><dd>{a.review_summary.provider_calls}</dd><dt>{t('Input / output tokens reported')}</dt><dd>{a.review_summary.prompt_tokens} / {a.review_summary.completion_tokens}</dd></dl></details>}
      ...
      <section className="paper-configuration">{comparisonForm}
